...
class myGraphics32(object):
    def __init__(self, widget, address, key , threshold = 1.5):
        ...
        #选取数据名称, 如果数据不存在, 默认为
        residual1 = self.dataset[["奇异谱熵"]].iloc[:, 0].values
        residual2 = self.dataset[["功率谱熵"]].iloc[:, 0].values
        ...
        p31 = widget.addPlot(labels = {'left': "奇异谱熵", })
        # p31.addLegend()
        x=np.arange(130)
        ...
        xx=np.arange(130)
        p32 = widget.addPlot( labels={'left': "功率谱熵", })
        p32.plot(x=xx, y=residual2, pen=(255, 0, 0), name="Red curve")
